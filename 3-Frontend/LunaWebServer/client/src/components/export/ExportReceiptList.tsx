...
import { ClipboardCheck, Package, Truck, CheckCircle2, MoreVertical } from "lucide-react";
...
interface ExportReceiptListProps {
  searchQuery: string;
  filters: {
    warehouse: string;
    status: string;
...
const mockReceipts: ExportReceipt[] = [
  {
    id: '5906469',
    batchId: '7C202',
    exportDate: '2024-10-10',
    exporter: 'Nguyễn Minh Hải',
    productCount: 125,
    totalValue: '45 M',
    status: 'transfering',
    warehouse: 'hcm',
...
function StatusBar({ status }: { status: ExportReceipt['status'] }) {
  const steps = [
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Chi tiết phiếu xuất #{receipt.id}</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="font-medium">{receipt.exportDate}</div>
            </div>
            <div>
              <div className="text-sm text-muted-foreground">Người xuất</div>
              <div className="font-medium">{receipt.exporter}</div>
            </div>
            <div>
...
interface CreateReceiptProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (receipt: Omit<ExportReceipt, 'id' | 'status'>) => void;
}
...
function CreateReceipt({ open, onOpenChange, onSubmit }: CreateReceiptProps) {
  ...
  const handleSubmit = () => {
    // Validate form data
    if (!formData.batchId || !formData.exporter || !formData.products.some(p => p.name && p.quantity > 0)) {
      return;
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Tạo phiếu xuất mới</DialogTitle>
        </DialogHeader>
        <DialogScrollArea className="max-h-[80vh]">
          <div className="space-y-4 p-1">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-sm text-muted-foreground">Ngày xuất</label>
                <input
                  type="date"
                  className="w-full p-2 border rounded"
              ...
              <div>
                <label className="text-sm text-muted-foreground">Người xuất</label>
                <input
                  type="text"
                  className="w-full p-2 border rounded"
                  value={formData.exporter}
                  onChange={e => setFormData(prev => ({ ...prev, exporter: e.target.value }))}
                />
              </div>
              <div>
...
export default function ExportReceiptList({
  ...
  const handleCreateReceipt = (newReceipt: Omit<ExportReceipt, 'id' | 'status'>) => {
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-x-8 gap-y-2 mt-2">
                    <div className="text-sm text-muted-foreground">Ngày xuất: {receipt.exportDate}</div>
                    <div className="text-sm text-muted-foreground">Số lượng sản phẩm: {receipt.productCount}</div>
                    <div className="text-sm text-muted-foreground">Người xuất: {receipt.exporter}</div>
